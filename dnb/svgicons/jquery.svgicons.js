...
5. To access an icon at a later point without using the callback, use this:
	$j.getSvgIcon(id (string));

This will return the icon (as jQuery object) with a given ID.
	
6. To resize icons at a later point without using the callback, use this:
	$j.resizeSvgIcons(resizeOptions) (use the same way as the "resize" parameter)
...
(function($j) {
	...
	$j.svgIcons = function(file, opts) {
		...
		var svgns = "http://www.w3.org/2000/svg",
			...
			if(opts.svgz) {
				var data_el = $j('<object data="' + file + '" type=image/svg+xml>').appendTo('body').hide();
				try {
						...
						if(window.opera) {
							...
							if(err.responseText) {
								try{
									...
									var xmlDoc = new window.ActiveXObject("Microsoft.XMLDOM");
									xmlDoc.async = "false";
									xmlDoc.loadXML(err.responseText);
									svgdoc = xmlDoc;
								}
		...
		function makeIcons(toImage, fallback) {
			...
			if(toImage) {
				var temp_holder = $j(document.createElement('div'));
				temp_holder.hide().appendTo('body');
			} 
			if(fallback) {
				var path = opts.fallback_path?opts.fallback_path:'';
				$j.each(fallback, function(id, imgsrc) {
					holder = $j('#' + id);
					var icon = $j(new Image())
						.attr({
							'class':'svg_icon',
							src: path + imgsrc,
							'width': icon_w,
			...
			if(opts.placement) {
				$j.each(opts.placement, function(sel, id) {
					if(!svg_icons[id]) return;
					$j(sel).each(function(i) {
						var copy = svg_icons[id].clone();
						if(i > 0 && !toImage) copy = fixIDs(copy, i, true);
						setIcon($j(this), copy, id);
					})
				});
			}
			...
			if(opts.resize) $j.resizeSvgIcons(opts.resize);
			icons_made = true;
		...
		fixIDs = function(svg_el, svg_num, force) {
			...
			id_elems.each(function(i) {
				var id = this.id;
				var no_dupes = ($j(svgdoc).find('#' + id).length <= 1);
...
// 				});
	...
	$j.getSvgIcon = function(id, uniqueClone) { 
	...
	$j.resizeSvgIcons = function(obj) {
		// FF2 and older don't detect .svg_icon, so we change it detect svg elems instead
		var change_sel = !$j('.svg_icon:first').length;
		$j.each(obj, function(sel, size) {
			var arr = $j.isArray(size);
