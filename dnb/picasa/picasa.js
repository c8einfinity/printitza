...
(function( $ ){
  ...
  $.fn.jqig = function( options ) {  
    ...
    var settings = {
      ...
      thumbsize: 160, // Thumbnail size can be varied by following value 32, 48, 64, 72, 104, 144, 150, 160
      selectedAlbum: {},
      albums: [],
    ...
    */
    function getAlbum(albumId, page) {
      ...
      var albumUrl = BASEURL + settings.username + "/albumid/"+albumId+"?kind=photo&access=public&alt=json&thumbsize="+ settings.thumbsize+"u" 
        + (( !settings.pagination.enable ) ? "" : "&max-results=" + settings.pagination.imgPerPage)
        + "&start-index="+ (settings.pagination.imgPerPage * (page-1) +1);
      
      // for debug mode
      $.getJSON(albumUrl, function(data) {createAlbum(data);Helper.hideLoader();});
      // hide JSON object
    ...
    */
    ...
    // init elements
      ...
      $j(".gallery-images-wrapper").mCustomScrollbar();
      // for Pagination
      if (settings.pagination.enable){
        ...
        var listPage = 3, listStart = -1, listEnd = -1;
      ...
      //img = $('<div class="image-wrapper"/>').append($('<a href="'+entry.content.src+'"/>').append($('<div class="img"/>').append($('<div class="mask"></div>')).append($('<img src="' + entry.media$group.media$thumbnail[0].url + '" />'))));
        var img = $('<div class="photo"/>').append($('<img src="' + entry.media$group.media$thumbnail[0].url + '" />'));
        if (settings.enableImageCaption) img.append($('<div class="image-caption"/>').html(entry.media$group.media$description.$t));
        imageContainer.append(img);
      });
      var imH = settings.thumbsize+parseFloat($('.image-wrapper img').css('margin-top'));
      var imW = settings.thumbsize+parseFloat($('.image-wrapper img').css('margin-left'));
    ...
    function getAlbums(){
      ...
      //$.getJSON(url, function(data) {createAlbums(data);});
		...
		.done(function() {
		console.log('done');
            Helper.hideLoader();//console.log( "second success" );
		})
		.fail(function() {
		console.log('fail');
			var type = svgEditor.uiStrings.elementLabel.PICASA_REQUEST_FAILED;
		  var	msg = svgEditor.uiStrings.elementLabel.USER_NOT_FOUND;
            Helper.hideLoader();
    ...
    function createAlbums(data) {
      if(data.feed && data.feed.entry && data.feed.entry.length){
        var items = [];
        var selector = $('<select class="select-small" id="picasa_album_selector">');
          $.each(data.feed.entry, function(keyentry, entry) {
