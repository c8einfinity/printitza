(function () {
    function r(i, z, t) {
        ...
        if (Ba)
            for (var B in z) i.setAttribute(B, z[B]);
        else
            for (B in z) {
                var W = z[B],
                    w = i[B];
                if (w && w.constructor === "SVGLength") w.baseVal.value = W;
    ...
    jQuery.fn.jGraduate = function (i) {
        ...
        return this.each(function () {
            function t(c, a, d, h, f) {
                ...
                } else g.appendChild(l); if (d === null) d = 1;
                f = r("path", {
                ...
                $j(X).mousedown(function (M) {
                    B(this);
                    R = F;
                    N.mousemove(la).mouseup(W).bind("touchmove",stopTouchMove);
                    S = ga.offset();
                    M.preventDefault();
                    return false
                }).data("stop", l).data("bg", f).dblclick(function () {
                    $j("div.jGraduate_LightBox").show();
                    for (var M = this, I = +l.getAttribute("stop-opacity") || 1, C = l.getAttribute("stop-color") || 1, Y = (parseFloat(I) * 255).toString(16); Y.length < 2;) Y = "0" + Y;
                    a = C.substr(1) + Y;
                    $j("#" + b + "_jGraduate_stopPicker").css({
                        left: 100,
                        bottom: 15
                    }).jPicker({
                        window: {
                        ...
                        images: {
                            clientPath: o.images.clientPath
                        },
                        color: {
                            active: a,
							//quickList: [new $j.jPicker.Color()],
                            alphaSupport: true
                        },
                        localization: svgEditor.uiStrings.colorPicker
                    }, function (Z) {
                        ...
                        $j("div.jGraduate_LightBox").hide();
                        $j("#" + b + "_jGraduate_stopPicker").hide()
                    }, null, function () {
                        $j("div.jGraduate_LightBox").hide();
                        $j("#" + b + "_jGraduate_stopPicker").hide()
                    })
            ...
            function W() {
                N.unbind("mousemove", la).unbind('touchmove',stopTouchMove);
            ...
            function la(c) {
                ...
                } else O.setAttribute("display", "none");
                R.setAttribute("transform", d);
                $j.data(R, "bg").setAttribute("transform", d);
                $j.data(R, "stop").setAttribute("offset", (a - 10) / j);
                var h = 0;
                $j(g).find("stop").each(function () {
            ...
            var e = $j(this),
                o = $j.extend(true, {}, jQuery.fn.jGraduateDefaults, i),
                b = e.attr("id"),
                s = "#" + e.attr("id") + " ";
            if (s) {
                var ma = function () {
                    ...
                    $j.isFunction(e.okCallback) && e.okCallback(e.paint);
                    e.hide()
                }, na = function () {
                        $j.isFunction(e.cancelCallback) && e.cancelCallback();
                        e.hide()
                    };
                $j.extend(true, e, {
                    paint: new $j.jGraduate.Paint({
                    ...
                    okCallback: $j.isFunction(z[1]) && z[1] || null,
                    cancelCallback: $j.isFunction(z[2]) && z[2] || null
                });
                e.position();
                var u = null,
                    N = $j(window);
                if (e.paint.type == "none") e.paint = $j.jGraduate.Paint({
                    solidColor: "ffffff"
                });
                e.addClass("jGraduate_Picker");
                e.html('<ul class="jGraduate_tabs"><li class="jGraduate_tab_color jGraduate_tab_current" data-type="col">Choose Color</li><li class="jGraduate_tab_lingrad" data-type="lg">Linear Gradient</li><li class="jGraduate_tab_radgrad" data-type="rg">Radial Gradient</li></ul><div class="jGraduate_colPick"></div><div class="jGraduate_gradPick"></div><div class="jGraduate_LightBox"></div><div id="' + b + '_jGraduate_stopPicker" class="jGraduate_stopPicker"></div>');
                var Ca = $j(s + "> .jGraduate_colPick"),
                    n = $j(s + "> .jGraduate_gradPick");
                n.html('<div id="' + b + '_jGraduate_Swatch" class="jGraduate_Swatch"><h2 class="jGraduate_Title">' + o.window.pickerTitle + '</h2><div id="' + b + '_jGraduate_GradContainer" class="jGraduate_GradContainer"></div><div id="' + b + '_jGraduate_StopSlider" class="jGraduate_StopSlider"></div></div><div class="jGraduate_Form jGraduate_Points jGraduate_lg_field"><div class="jGraduate_StopSection"><label class="jGraduate_Form_Heading">Begin Point</label><div class="jGraduate_Form_Section"><label>x:</label><input type="text" id="' +
                    ...
                    b + '_jGraduate_y2" size="3" title="Enter ending y value between 0.0 and 1.0"/></div></div></div><div class="jGraduate_Form jGraduate_Points jGraduate_rg_field"><div class="jGraduate_StopSection"><label class="jGraduate_Form_Heading">Center Point</label><div class="jGraduate_Form_Section"><label>x:</label><input type="text" id="' + b + '_jGraduate_cx" size="3" title="Enter x value between 0.0 and 1.0"/><label> y:</label><input type="text" id="' + b + '_jGraduate_cy" size="3" title="Enter y value between 0.0 and 1.0"/></div></div><div class="jGraduate_StopSection focalPoint"><label class="jGraduate_Form_Heading">Focal Point</label><div class="jGraduate_Form_Section"><label>Match center: <input type="checkbox" checked="checked" id="' +
                    b + '_jGraduate_match_ctr"/></label><label>x:</label><input type="text" id="' + b + '_jGraduate_fx" size="3" title="Enter x value between 0.0 and 1.0"/><label> y:</label><input type="text" id="' + b + '_jGraduate_fy" size="3" title="Enter y value between 0.0 and 1.0"/></div></div></div><div style="display:none" class="jGraduate_StopSection jGraduate_SpreadMethod"><label class="jGraduate_Form_Heading">Spread method</label><div class="jGraduate_Form_Section"><select class="jGraduate_spreadMethod"><option value=pad selected>Pad</option><option value=reflect>Reflect</option><option value=repeat>Repeat</option></select></div></div><div class="jGraduate_Form"><div class="jGraduate_Slider jGraduate_RadiusField jGraduate_rg_field"><label class="prelabel">Radius:</label><div id="' +
                    ...
                    b + '_jGraduate_Ok" class="jGraduate_Ok" value="OK"/><input type="button" id="' + b + '_jGraduate_Cancel" class="jGraduate_Cancel" value="Cancel"/></div>');
                var j = 256,
                    oa = j - 0,
                    pa = j - 0,
                    p, g, aa, q = {};
                $j(".jGraduate_SliderBar").width(145);
                var x = $j("#" + b + "_jGraduate_GradContainer")[0],
                    m = r("svg", {
                        id: b + "_jgraduate_svg",
                        width: j,
                        height: j,
                ...
                case "radialGradient":
                    ...
                    r("linearGradient", {
                        id: b + "_lg_jgraduate_grad"
                    }, m)
                }
                if (ba) {
                    v = g = $j("#" + b + "_lg_jgraduate_grad")[0];
                    u = e.paint[p];
                    t(0, "#" + u, 1);
                    var K = typeof o.newstop;
                    if (K === "string") switch (o.newstop) {
                    case "same":
                        t(1, "#" + u, 1);
                        break;
                    case "inverse":
                    ...
                    case "black":
                        t(1, "#000000", 1)
                    } else if (K === "object") t(1, o.newstop.color || "#" + u, "opac" in o.newstop ? o.newstop.opac : 1)
                }
                ...
                aa = r("rect", {
                    id: b + "_jgraduate_rect",
                    x: 0,
                    y: 0,
                    width: oa,
                    height: pa,
                    fill: "url(#" + b + "_jgraduate_grad)",
                    "fill-opacity": U / 100
                }, m);
                var sa = $j("<div/>").attr({
                ...
                $j(s + " .grad_coord");
                $j.each(["x1", "y1", "x2", "y2", "cx", "cy", "fx", "fy"], function (c, a) {
                    var d = g.getAttribute(a),
                        h = isNaN(a[1]);
                    d || (d = h ? "0.5" : a === "x2" ? "1.0" : "0.0");
                    q[a] = $j("#" + b + "_jGraduate_" + a).val(d).change(function () {
                        if (isNaN(parseFloat(this.value)) || this.value < 0) this.value = 0;
                        else if (this.value > 1) this.value = 1;
                        if (!(a[0] === "f" && !E))
                            if (h && p === "radialGradient" || !h && p === "linearGradient") g.setAttribute(a,
                                this.value);
                ...
                var D, fa, ga = $j("#" + b + "_jGraduate_StopSlider"),
                    F, H, R, O = r("path", {
                ...
                r("image", {
                    width: 16,
                    height: 16
                }, x).setAttributeNS(A.xlink,
                    "xlink:href", o.images.clientPath + "map-opacity.png");
                $j(H).click(function (c) {
                    S = ga.offset();
                ...
                var ua = n.find(".jGraduate_spreadMethod").change(function () {
                    alert('hi');
                    g.setAttribute("spreadMethod", $j(this).val())
                }),
                    Q = null,
                    va = function (c) {
                        ...
                        function () {
                            Q = null;
                            N.unbind("mousemove", va).unbind("mouseup", wa).unbind('touchmove',stopTouchMove);
                    };
                ...
                var k = D.length;
                for (y = 0; y < k; y++) t(0, 0, 0, 0, D[y]);
                ua.val(g.getAttribute("spreadMethod") || "pad");
                var ja, E = false;
                aa.setAttribute("fill-opacity", U / 100);
                $j("#" + b + " div.grad_coord").mousedown(function (c) {
                    c.preventDefault();
                    Q = $j(this);
                    Q.offset();
                    ja = Q.parent().offset();
                    N.mousemove(va).mouseup(wa).bind("touchmove",stopTouchMove);
                });
                $j("#" + b + "_jGraduate_Ok").bind("click", function () {
                    e.paint.type = p;
                    e.paint[p] = g.cloneNode(true);
                    e.paint.solidColor = null;
                    ma()
                });
                $j("#" + b + "_jGraduate_Cancel").bind("click", function () {
                    na()
                });
                if (p === "radialGradient")
                    if (E) V.show();
                ...
                $j("#" + b + "_jGraduate_match_ctr")[0].checked = !E;
                var xa, ya;
                $j("#" + b + "_jGraduate_match_ctr").change(function () {
                    E = !this.checked;
                    V.toggle(E);
                    q.fx.val("");
                    q.fy.val("");
                ...
                if (p === "radialGradient") {
                    ...
                    if (m.numberOfItems === 2) {
                        ...
                        if (k.type === 2 && m.type === 3) {
                            k = m.matrix;
                            if (k.a !== 1) n = Math.round(-(1 - k.a) * 100);
                            else if (k.d !==
                                1) n = Math.round((1 - k.d) * 100)
                        }
                        ...
                        if (x.type === 4 && k.type === 2 && m.type === 3) {
                            ...
                            k = m.matrix;
                            if (k.a !== 1) n = Math.round(-(1 - k.a) * 100);
                            else if (k.d !== 1) n = Math.round((1 - k.d) * 100)
                        }
                    }
                }
                n = {
                    radius: {
                        handle: "#" + b + "_jGraduate_RadiusArrows",
                        input: "#" + b + "_jGraduate_RadiusInput",
                        val: (g.getAttribute("r") || 0.5) * 100
                    },
                    opacity: {
                        handle: "#" + b + "_jGraduate_OpacArrows",
                        input: "#" + b + "_jGraduate_OpacInput",
                        val: e.paint.alpha || 100
                    },
                    ellip: {
                        handle: "#" +
                            b + "_jGraduate_EllipArrows",
                        input: "#" + b + "_jGraduate_EllipInput",
                        val: n
                    },
                    angle: {
                        handle: "#" + b + "_jGraduate_AngleArrows",
                        input: "#" + b + "_jGraduate_AngleInput",
                        val: U
                    }
                };
                $j.each(n, function (c, a) {
                    var d = $j(a.handle);
                    d.mousedown(function (h) {
                        ...
                        N.mousemove(za).mouseup(Aa).bind("touchmove",stopTouchMove);
                        h.preventDefault()
                    });
                    $j(a.input).val(a.val).change(function () {
                        var h = +this.value,
                            f = 0,
                            l = p === "radialGradient";
                        switch (c) {
                ...
                var za = function (c) {
                    ...
                    c.preventDefault()
                        ...
                        N.unbind("mousemove", za).unbind("mouseup", Aa).unbind("touchmove", stopTouchMove);
                        L = null
                    };
                for (n =
                    (e.paint.alpha * 255 / 100).toString(16); n.length < 2;) n = "0" + n;
                n = n.split(".")[0];
                u = e.paint.solidColor == "none" ? "" : e.paint.solidColor + n;
                ba || (u = D[0].getAttribute("stop-color"));
                $j.extend($j.fn.jPicker.defaults.window, {
                    alphaSupport: true,
                    effects: {
                        type: "show",
                        speed: 0
                ...
                Ca.jPicker({
                    ...
                    e.paint.solidColor =
                        c.val("hex") ? c.val("hex") : "none";
                    e.paint.radialGradient = null;
                    ma()
                }, null, function () {
                    na()
                });
                var ka = $j(s + " .jGraduate_tabs li");
                ka.click(function () {
                    ka.removeClass("jGraduate_tab_current");
                    $j(this).addClass("jGraduate_tab_current");
                    $j(s + " > div").hide();
                    var c = $j(this).attr("data-type");
                    $j(s + " .jGraduate_gradPick").show();
                    if (c === "rg" || c === "lg") {
                        $j(".jGraduate_" + c + "_field").show();
                        $j(".jGraduate_" + (c === "lg" ? "rg" : "lg") + "_field").hide();
                        $j("#" + b + "_jgraduate_rect")[0].setAttribute("fill", "url(#" + b + "_" + c + "_jgraduate_grad)");
                        p = c === "lg" ? "linearGradient" : "radialGradient";
                        $j("#" + b + "_jGraduate_OpacInput").val(e.paint.alpha).change();
                        var a = $j("#" + b + "_" + c + "_jgraduate_grad")[0];
                        if (g !== a) {
                            var d = $j(g).find("stop");
                            $j(a).empty().append(d);
                            g = a;
                        ...
                        $j("#" + b + "_jGraduate_focusCoord").toggle(E);
                        if (E) $j("#" + b + "_jGraduate_match_ctr")[0].checked = false
                    } else {
                        $j(s + " .jGraduate_gradPick").hide();
                        $j(s + " .jGraduate_colPick").show()
                    }
                });
                $j(s + " > div").hide();
                ka.removeClass("jGraduate_tab_current");
                ...
                switch (e.paint.type) {
                case "linearGradient":
                    ea = $j(s + " .jGraduate_tab_lingrad");
                    break;
                case "radialGradient":
                    ea = $j(s + " .jGraduate_tab_radgrad");
                    break;
                default:
                    ea = $j(s + " .jGraduate_tab_color")
                }
                e.show();
                setTimeout(function () {
