...
//            if ($colBrakes && count($colBrakes) && $colBrakes[$counter]['colbrake']) {
            ...
            $is3dCategory = $child->getisPramotionalCategory(); //is_pramotional_category
            if($is3dCategory == true)
            {
                $pramotional3dSpan = ' <span class="pramotinal-category">3D</span>';
            }else{
                $pramotional3dSpan = '';
            }
            ...
            $html .= '<a' . $openInNewTab . ' href="' . $child->getUrl() . '" ' . $outermostClassCode . '><span>' . $this->escapeHtml(
                    $child->getName()
                ) . '</span> '.$pramotional3dSpan.' </a>' . $this->_addSubMenu(
                    $child,
                    $childLevel,
                    $childrenWrapClass,
                    $limit
...
//        if ($colBrakes && count($colBrakes) && $limit) {
    ...
    protected function _getMenuItemClasses(\Magento\Framework\Data\Tree\Node $item)
    {
        ...
        if ($item->getLevel() == 0) {
            if($item->getId() == 'designnbuy-designer-designs'){
                $item->setData('weltpixel_mm_display_mode','fullwidth');
            }
