    ...
    $.widget('bss.fastorder_option', {
        ...
        selectProduct: function (el) {
            var productSku = $(el).find('.bss-product-sku-select').val(),
                ...
                absolute_price = $(el).find('.bss-product-absolute-price').val(),
            ...
            if($(el).find('.bss-show-popup').val() == 0){
                $(rowEl).find('.bss-fastorder-row-qty input.qty').removeAttr('readonly');
            }
            if($(el).find('.bss-product-attachment-select').val() == 1){
                $(rowEl).find('.bss-fastorder-row-attachment .product-options').show();
            } else{
                $(rowEl).find('.bss-fastorder-row-attachment .product-options').hide();
            }
            ...
            $(rowEl).find('.bss-fastorder-row-qty .bss-product-price-absolute-price').val(absolute_price);
        ...
        selectOption: function (sortOrder) {
            ...
            var mageworx_qty = jQuery(".bss-options-info .control .swatch-attribute-options.quantity div.selected").attr('data-option-label');
            
            if (typeof mageworx_qty != 'undefined' && mageworx_qty != null) {
                $('#bss-fastorder-'+sortOrder).find('.bss-fastorder-row-qty input.qty').val(mageworx_qty);
			} else {
                var mageworx_qty_select = jQuery(".bss-options-info .control .product-custom-option.quantity option:selected").text();
                
                if (typeof mageworx_qty_select != 'undefined' && mageworx_qty_select != '') {
                    $('#bss-fastorder-'+sortOrder).find('.bss-fastorder-row-qty input.qty').val(mageworx_qty_select);
                } else {
            ...
            switch (optionType) {
                ...
                case 'select-one':
                    ...
                    if (element.attr('data-incl-tax')) {
                        ...
                        price = element.find(":selected").attr('fast_price');
