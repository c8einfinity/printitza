 ...
 */
namespace Lumaplaza\Calculatorshipping\Helper;
...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
	...
	public function showShippingEstimate(){
		return $this->_layoutFactory->create()->createBlock('Lumaplaza\Calculatorshipping\Block\Calculatorshipping')
    ...
    public function getLocation(){ 
        ...
        $query = @unserialize(file_get_contents('http://ip-api.com/php/'.$this->getIpAddr()));
        
        $result = array();
        if($query && $query['status'] == 'success') {
            if(isset($query['countryCode'])){
                $result['country_code'] = $query['countryCode'];
            }
            if(isset($query['regionName'])){
                $result['region_name'] = $query['regionName'];
            }
            if(isset($query['region'])){
                $result['region_code'] = $query['region'];
            }
            if(isset($query['zip'])){
                $result['zip_code'] = $query['zip'];
            }
        }
