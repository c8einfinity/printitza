...
class AddGroupOptionToProductObserver implements \Magento\Framework\Event\ObserverInterface
{
    ...
    protected function apply22xCompatibilityFix($modProductOptions, $post)
    {
        ...
        foreach ($modProductOptions as $optionKey => $optionData) {
            if (!empty($post['options_use_default'])) {
                ...
                foreach ($useDefaults as $useDefaultOptionId => $useDefaultOptionData) {
                    ...
                    $this->processValues($optionData, $values, $useDefaultOptionData, $mageworxOptionTypeIds);
                ...
                foreach ($values as $valueKey => $value) {
                    $values[$valueKey]['option_type_id'] = null;

                    if (isset($value['mageworx_option_type_id']) && !$this->isImportedOption($optionData)) {
        ...
        if (!empty($mageworxOptionTypeIds)) {
            $condition = 'mageworx_option_type_id IN (' . implode(',', $mageworxOptionTypeIds) . ')';
     ...
     * @param array $mageworxOptionTypeIds
     * @return void
     */
    protected function processValues($optionData, &$values, $useDefaultOptionData, &$mageworxOptionTypeIds)
    {
        foreach ($values as $valueKey => $value) {
