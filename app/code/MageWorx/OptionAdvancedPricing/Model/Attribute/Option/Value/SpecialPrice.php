...
class SpecialPrice extends AbstractAttribute implements AttributeInterface
{
    ...
    public function collectData($entity, array $options)
    {
        ...
        foreach ($options as $option) {
            ...
            foreach ($option['values'] as $value) {
                ...
                $specialPrices[$value['mageworx_option_type_id']] = $value[$this->getName()];
            }
        }
    ...
    protected function collectSpecialPrices($items)
    {
        ...
        foreach ($items as $itemKey => $itemValue) {
            $data['delete'][] = [
                SpecialPriceModel::COLUMN_MAGEWORX_OPTION_TYPE_ID => $itemKey,
            ];
            ...
            foreach ($decodedJsonData as $dataItem) {
                ...
                $comment   = htmlspecialchars(
                    $dataItem[SpecialPriceModel::COLUMN_COMMENT],
                    ENT_COMPAT,
                    'UTF-8',
                    false
                );
    ...
    public function deleteOldData(array $data)
    {
        ...
        foreach ($data as $dataItem) {
            $mageworxOptionValueIds[] = $dataItem[SpecialPriceModel::COLUMN_MAGEWORX_OPTION_TYPE_ID];
        }
        if (!$mageworxOptionValueIds) {
            return;
        }
        $tableName  = $this->resource->getTableName($this->getTableName());
        $conditions = SpecialPriceModel::COLUMN_MAGEWORX_OPTION_TYPE_ID .
    ...
    public function processDuplicate($newMageworxId, $oldMageworxId, $entityType = 'product')
    {
        ...
        $select = $connection->select()->from(
            ...
            SpecialPriceModel::COLUMN_MAGEWORX_OPTION_TYPE_ID . ' = ?',
            $oldMageworxId
        );
