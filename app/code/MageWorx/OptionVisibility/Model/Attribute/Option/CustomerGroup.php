...
class CustomerGroup extends AbstractAttribute
{
    ...
    public function collectData($entity, array $options)
    {
        ...
        foreach ($options as $option) {
            ...
            $customerGroups[$option['mageworx_option_id']] = $option[$this->getName()];
        }
    ...
    protected function collectCustomerGroup($items)
    {
        ...
        foreach ($items as $itemKey => $itemValue) {
            $data['delete'][] = [
                CustomerGroupModel::COLUMN_NAME_MAGEWORX_OPTION_ID => $itemKey,
            ];
    ...
    public function deleteOldData(array $data)
    {
        ...
        foreach ($data as $dataItem) {
            $mageworxOptionValueIds[] = $dataItem[CustomerGroupModel::COLUMN_NAME_MAGEWORX_OPTION_ID];
        }
        if (!$mageworxOptionValueIds) {
            return;
        }
        $tableName  = $this->resource->getTableName($this->getTableName());
        $conditions = CustomerGroupModel::COLUMN_NAME_MAGEWORX_OPTION_ID .
    ...
    public function processDuplicate($newMageworxId, $oldMageworxId, $entityType = 'product')
    {
        ...
        $select = $connection->select()->from(
            ...
            CustomerGroupModel::COLUMN_NAME_MAGEWORX_OPTION_ID . ' = ?',
            $oldMageworxId
        );
