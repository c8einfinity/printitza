...
class StoreView extends AbstractAttribute
{
    ...
    public function collectData($entity, array $options)
    {
        ...
        foreach ($options as $option) {
            ...
            $storeGroups[$option['mageworx_option_id']] = $option[$this->getName()];
        }
    ...
    protected function collectStoreView($items)
    {
        ...
        foreach ($items as $itemKey => $itemValue) {
            $data['delete'][] = [
                StoreViewModel::COLUMN_NAME_MAGEWORX_OPTION_ID => $itemKey,
            ];
    ...
    public function deleteOldData(array $data)
    {
        ...
        foreach ($data as $dataItem) {
            $mageworxOptionValueIds[] = $dataItem[StoreViewModel::COLUMN_NAME_MAGEWORX_OPTION_ID];
        }
        if (!$mageworxOptionValueIds) {
            return;
        }
        $tableName  = $this->resource->getTableName($this->getTableName());
        $conditions = StoreViewModel::COLUMN_NAME_MAGEWORX_OPTION_ID .
    ...
    public function processDuplicate($newMageworxId, $oldMageworxId, $entityType = 'product')
    {
        ...
        $select = $connection->select()->from(
            ...
            StoreViewModel::COLUMN_NAME_MAGEWORX_OPTION_ID . ' = ?',
            $oldMageworxId
        );
