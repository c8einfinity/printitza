...
class Inkscape
{
    ...
    public function __construct( $filename )
    {
		...
		/*$svgfileContents = file_get_contents($filename);
		$doc = new \DOMDocument();
		$doc->preserveWhiteSpace = False;
		$doc->loadXML($svgfileContents);
		$tags = $doc->getElementsByTagName('svg')->item(0);
		
		if($tags->getAttribute('width') > $tags->getAttribute('height')){
			$this->_width = 500;
		}else{
			$this->_height = 500;
		}
    ...
    /* public function setSize( $width, $height )
    {
        ...
        if ( $height )
        {
            $this->addParam( 'export-height', $height );
        }
    } */
	public function setSize( $width, $height )
    {
       if(isset($width) || isset($height)){
			...
			{
				$this->addParam( 'export-width', $this->_width );
			}
			if ( isset( $this->_height) )
			{
				$this->addParam( 'export-height', $this->_height );
			}
		}
