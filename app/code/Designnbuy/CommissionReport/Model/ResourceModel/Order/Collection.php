...
namespace Designnbuy\CommissionReport\Model\ResourceModel\Order;
...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    /**
     * @var intapp/code/Designnbuy/CommissionReport/Model/ResourceModel/Order/Collection.php
    ...
    /**
     * Add Commission filter to collection
     * @param array|int|string  $commission
     * @return $this
     */
    public function addCommissionsFilter($commissionIds)
    {
        if (!is_array($commissionIds)) {
            $commissionIds = explode(',', $commissionIds);
            foreach ($commissionIds as $key => $id) {
                $id = trim($id);
                if (!$id) {
                    unset($commissionIds[$key]);
                }
            }
        }

        if (!count($commissionIds)) {
            $commissionIds = [0];
        }
