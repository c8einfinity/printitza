 ...
 * SquareArea model
 *
 * @method \Designnbuy\PrintingMethod\Model\ResourceModel\SquareArea _getResource()
 * @method \Designnbuy\PrintingMethod\Model\ResourceModel\SquareArea getResource()
 ...
 */
class SquareArea extends \Magento\Framework\Model\AbstractModel
{
    /**
     * SquareAreas's Statuses
     */
     ...
     */
    const BASE_MEDIA_PATH = 'designnbuy'. DIRECTORY_SEPARATOR .'SquareArea';
     ...
     * @var string
     */
    protected $_eventPrefix = 'designnbuy_printingmethod_squarearea';
     ...
     * @var string
     */
    protected $_eventObject = 'printingmethod_squarearea';
     ...
     */
    protected $_relatedSquareAreasCollection;
    ...
    public function __construct(
        ...
        $this->_relatedSquareAreasCollection = clone($this->getCollection());
    }
    ...
    {
        $this->_init('Designnbuy\PrintingMethod\Model\ResourceModel\SquareArea');
    }
    ...
    {
        return $plural ? 'Square Areas' : 'Square Area';
    }

    /**
     * Retrieve true if squarearea is active
    ...
    /**
     * Retrieve available squarearea statuses
     * @return array
     */
    ...
    /**
     * Check if squarearea identifier exist for specific store
     * return squarearea id if squarearea exists
     *
    ...
    /**
     * Retrieve squarearea url route path
     * @return string
     */
    ...
    /**
     * Retrieve squarearea url
     * @return string
     */
    public function getSquareAreaUrl()
    {
        if (!$this->hasData('squarearea_url')) {
            $url = $this->_url->getUrl($this, URL::CONTROLLER_PRINTINGMETHOD);
            $this->setData('squarearea_url', $url);
        }

        return $this->getData('squarearea_url');
    }
    ...
    {
        if (!$this->hasData('squarearea_image')) {
            if ($file = $this->getData('image')) {
                $image = $this->_url->getMediaUrl($file);
            } else {
                $image = false;
            }
            $this->setData('squarearea_image', $image);
        }

        return $this->getData('squarearea_image');
    }
    ...
    /**
     * Retrieve squarearea related products
    ...
    public function getRelatedProducts()
    {
        if (!$this->hasData('related_products')) {
            ...
            $collection->getSelect()->joinLeft(
                ['rl' => $this->getResource()->getTable('designnbuy_printingmethod_squarearea_relatedproduct')],
    ...
    /**
     * Retrieve squarearea publish date using format
     * @param  string $format
     * @return string
     */
    ...
    /**
     * Temporary method to get images from some custom squarearea version. Do not use this method.
     * @param  string $format
     * @return string
     */
    public function getSquareAreaImage()
    {
        $image = $this->getData('image');
        if (!$image) {
            $image = $this->getData('squarearea_image');
        }
        return $image;
    }
    ...
    /**
     * Duplicate squarearea and return new object
     * @return self
     */
