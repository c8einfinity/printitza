 ...
 */
namespace Designnbuy\DesignReview\Block\Adminhtml\Edit;
...
class Form extends \Magento\Backend\Block\Widget\Form\Generic
{
     ...
     * Template factory
     *
     * @var \Designnbuy\Template\Model\DesignideaFactory
     */
    ...
    protected function _prepareForm()
    {
        ...
        if($review->getEntityId() == 1){
            $design = $this->_designideaFactory->create()->load($review->getEntityPkValue());
            $url = $this->getUrl(
                'designidea/designidea/edit',
                ['id' => $design->getId()]
            );
        } elseif ($review->getEntityId() == 2){
            $design = $this->_templateFactory->create()->load($review->getEntityPkValue());
            $url = $this->getUrl(
        ...
        $fieldset->addField(
            ...
            [
                'label' => __('Product'),
                'text' => '<a href="' . $url . '" onclick="this.target=\'blank\'">' . $this->escapeHtml(
                        $design->getTitle()
                ) . '</a>'
            ]
        );
        ...
        $fieldset->addField(
            ...
            [
                ...
                'text' => $this->getLayout()->createBlock(
                    \Designnbuy\DesignReview\Block\Adminhtml\Rating\Summary::class
                )->toHtml()
            ]
        );
        ...
        $fieldset->addField(
            ...
            [
                ...
                'text' => '<div id="rating_detail">' . $this->getLayout()->createBlock(
                    \Designnbuy\DesignReview\Block\Adminhtml\Rating\Detailed::class
                )->toHtml() . '</div>'
            ]
        );
