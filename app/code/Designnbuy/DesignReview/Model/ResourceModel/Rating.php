 ...
 */
namespace Designnbuy\DesignReview\Model\ResourceModel;
...
class Rating extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
     ...
     * @param \Designnbuy\DesignReview\Model\ResourceModel\Review\Summary $reviewSummary
    ...
    public function __construct(
        ...
        \Designnbuy\DesignReview\Model\ResourceModel\Review\Summary $reviewSummary,
        $connectionName = null
    ) {
    ...
    {
        $this->_init('design_rating', 'rating_id');
    }
     ...
     * @param string $field
     * @param mixed $value
     * @param \Designnbuy\DesignReview\Model\Rating $object
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        $select->joinLeft(
            ['title' => $this->getTable('design_rating_title')],
     ...
     * Actions after load
     *
     * @param \Magento\Framework\Model\AbstractModel|\Designnbuy\DesignReview\Model\Rating $object
     * @return $this
     */
    protected function _afterLoad(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('design_rating_title'),
            ['store_id', 'value']
        )->where(
            'rating_id=:rating_id'
        );
    ...
    public function getStores($ratingId)
    {
        $select = $this->getConnection()->select()->from(
            $this->getTable('design_rating_store'),
            'store_id'
        )->where(
            'rating_id = ?',
            $ratingId
    ...
    protected function processRatingCodes(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $table = $this->getTable('design_rating_title');
    ...
    protected function processRatingStores(\Magento\Framework\Model\AbstractModel $object)
    {
        $connection = $this->getConnection();
        $ratingId = (int)$object->getId();
        $table = $this->getTable('design_rating_store');
    ...
    protected function _afterDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        if (!$this->moduleManager->isEnabled('Designnbuy_DesignReview')) {
            return $this;
        }
    ...
    protected function _getEntitySummaryData($object)
    {
        ...
        $select = $connection->select()->from(
            ['rating_vote' => $this->getTable('design_rating_option_vote')],
            ['entity_pk_value' => 'rating_vote.entity_pk_value', 'sum' => $sumColumn, 'count' => $countColumn]
        )->join(
            ['review' => $this->getTable('design_review')],
            'rating_vote.review_id=review.review_id',
            []
        )->joinLeft(
            ['review_store' => $this->getTable('design_review_store')],
            'rating_vote.review_id=review_store.review_id',
            ['review_store.store_id']
        );
        if (!$this->_storeManager->isSingleStoreMode()) {
            $select->join(
                ['rating_store' => $this->getTable('design_rating_store')],
                'rating_store.rating_id = rating_vote.rating_id AND rating_store.store_id = review_store.store_id',
                []
            );
        }
        $select->join(
            ['design_review_status' => $this->getTable('design_review_status')],
            'review.status_id = design_review_status.status_id',
            []
        )->where(
            'design_review_status.status_code = :status_code'
        )->group(
    ...
    {
        $entityId = $this->getEntityIdByCode(\Designnbuy\DesignReview\Model\Rating::ENTITY_PRODUCT_CODE);
        $connection = $this->getConnection();
        ...
        if ($ratingIds) {
            ...
            $connection->delete($this->getTable('design_rating_option_vote_aggregated'), $where);
        }
