...
class Review extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
     ...
     *
     * @var \Designnbuy\DesignReview\Model\ResourceModel\Rating\Option
     */
     ...
     * @param \Designnbuy\DesignReview\Model\ResourceModel\Rating\Option $ratingOptions
    ...
    {
        $this->_init('design_review', 'review_id');
        $this->_reviewTable = $this->getTable('design_review');
        $this->_reviewDetailTable = $this->getTable('design_review_detail');
        $this->_reviewStatusTable = $this->getTable('design_review_status');
        $this->_reviewEntityTable = $this->getTable('design_review_entity');
        $this->_reviewStoreTable = $this->getTable('design_review_store');
        $this->_aggregateTable = $this->getTable('design_review_entity_summary');
    }
    ...
    public function getTotalReviews($entityPkValue, $approvedOnly = false, $storeId = 0)
    {
        ...
        if ($approvedOnly) {
            ...
            $bind[':status_id'] = \Designnbuy\DesignReview\Model\Review::STATUS_APPROVED;
        }
     ...
     *
     * @param \Magento\Framework\Model\AbstractModel $object
     * @param \Designnbuy\DesignReview\Model\Rating $ratingSummaryObject
     * @return void
     */
    ...
    protected function _loadVotedRatingIds($reviewId)
    {
        ...
        $select = $connection->select()->from(['v' => $this->getTable('design_rating_option_vote')], 'r.rating_id')
            ->joinInner(['r' => $this->getTable('design_rating')], 'v.rating_id=r.rating_id')
    ...
    public function deleteReviewsByProductId($productId)
    {
        $this->getConnection()->delete(
            ...
            [
                ...
                'entity_id=?' => $this->getEntityIdByCode(\Designnbuy\DesignReview\Model\Review::ENTITY_PRODUCT_CODE)
            ]
        );
        $this->getConnection()->delete(
            $this->getTable('design_review_entity_summary'),
            [
                'entity_pk_value=?' => $productId,
                'entity_type=?' => $this->getEntityIdByCode(\Designnbuy\DesignReview\Model\Review::ENTITY_PRODUCT_CODE)
            ]
        );
        return $this;
    }
