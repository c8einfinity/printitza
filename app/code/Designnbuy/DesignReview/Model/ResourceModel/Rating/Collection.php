...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    /**
     * @var \Designnbuy\DesignReview\Model\ResourceModel\Rating\Option\CollectionFactory
     */
     ...
     * @param \Designnbuy\DesignReview\Model\ResourceModel\Rating\Option\CollectionFactory $ratingCollectionF
    ...
    public function __construct(
        ...
        \Designnbuy\DesignReview\Model\ResourceModel\Rating\Option\CollectionFactory $ratingCollectionF,
    ...
    {
        $this->_init(\Designnbuy\DesignReview\Model\Rating::class, \Designnbuy\DesignReview\Model\ResourceModel\Rating::class);
    }
    ...
    public function addEntityFilter($entity)
    {
        ...
        $this->getSelect()->join(
            ...
            'main_table.entity_id=' . $this->getTable('design_rating_entity') . '.entity_id',
            ['entity_code']
        );
        ...
        if (is_numeric($entity)) {
            $this->addFilter(
                'entity',
                $connection->quoteInto($this->getTable('design_rating_entity') . '.entity_id=?', $entity),
                'string'
            );
        } elseif (is_string($entity)) {
            $this->addFilter(
                'entity',
                $connection->quoteInto($this->getTable('design_rating_entity') . '.entity_code=?', $entity),
                'string'
            );
        }
        return $this;
    ...
    public function setStoreFilter($storeId)
    {
        ...
        if (!$this->_isStoreJoined) {
            $this->getSelect()->distinct(
                ...
                ['store' => $this->getTable('design_rating_store')],
    ...
    public function addOptionToItems()
    {
        ...
        if (!empty($arrRatingId)) {
            /** @var \Designnbuy\DesignReview\Model\ResourceModel\Rating\Option\Collection $collection */
    ...
    public function addEntitySummaryToItem($entityPkValue, $storeId)
    {
        ...
        $select = $connection->select()->from(
            ['rating_option_vote' => $this->getTable('design_rating_option_vote')],
        ...
        if (!$this->_storeManager->isSingleStoreMode()) {
            $select->join(
                ['rst' => $this->getTable('design_rating_store')],
    ...
    protected function _addStoreData()
    {
        ...
        $this->_select = $connection->select()->from($this->getTable('design_rating_store'))->where($inCondition);
