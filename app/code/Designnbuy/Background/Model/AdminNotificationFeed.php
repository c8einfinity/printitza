...
/**
 * Background admin notification feed model
 */
class AdminNotificationFeed extends \Magento\AdminNotification\Model\Feed
{
    ...
    public function getFeedUrl()
    {
        ...
        $modulesParams = [];
        foreach($this->getDesignnbuyModules() as $key => $module) {
            $key = str_replace('Designnbuy_', '', $key);
    ...
    /**
     * Get Designnbuy Modules Info
     *
     * @return $this
     */
    protected function getDesignnbuyModules()
    {
        $modules = [];
        foreach($this->_moduleList->getAll() as $moduleName => $module) {
            if ( strpos($moduleName, 'Designnbuy_') !== false && $this->_moduleManager->isEnabled($moduleName) ) {
    ...
    {
        return $this->_cacheManager->load('designnbuy_admin_notifications_lastcheck');
    }
    ...
    {
        $this->_cacheManager->save(time(), 'designnbuy_admin_notifications_lastcheck');
        return $this;
    }
