...
class PageIdentifierGenerator
{
    ...
    public function generate(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        while (true) {
            ...
            $backgroundId = $background->checkIdentifier($finalIdentifier, $object->getStoreId());
            $categoryId = $category->checkIdentifier($finalIdentifier, $object->getStoreId());

            if (!$backgroundId && !$categoryId) {
                break;
            } else {
                if ($backgroundId
                    && $backgroundId == $object->getId()
                    && $object instanceof \Designnbuy\Background\Model\Background
                ) {
                    break;
                }
                ...
                if ($categoryId
                    ...
                    && $object instanceof \Designnbuy\Background\Model\Category
                ) {
                    break;
                }
            }
