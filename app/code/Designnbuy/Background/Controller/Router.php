...
namespace Designnbuy\Background\Controller;

use \Designnbuy\Background\Model\Url;
/**
 * Background Controller Router
 */
class Router implements \Magento\Framework\App\RouterInterface
{
     ...
     * @param \Designnbuy\Background\Model\TagFactory $tagFactory
    ...
    /**
     * Validate and Match Background Pages and modify request
     *
    ...
    public function match(\Magento\Framework\App\RequestInterface $request)
    {
        ...
        $backgroundRoute = $this->_url->getRoute();

        if ($pathInfo[0] != $backgroundRoute) {
            return;
        }
        unset($pathInfo[0]);
            ...
            case Url::PERMALINK_TYPE_SHORT:
                if (isset($pathInfo[1])) {
                    if ($pathInfo[1] == $this->_url->getRoute(Url::CONTROLLER_SEARCH)) {
                        ...
                        } elseif ($backgroundId = $this->_getBackgroundId($pathInfo[1])) {
                            $pathInfo[2] = $pathInfo[1];
                        ...
                        if ($backgroundId = $this->_getBackgroundId(implode('/', $pathInfo))) {
        ...
        $this->_eventManager->dispatch(
            'designnbuy_background_controller_router_match_before',
        ...
        if (!$identifier) {
            $request->setModuleName('background')->setControllerName('index')->setActionName('index');
            $success = true;
        } elseif (count($info) > 1) {
            ...
            switch ($info[0]) {
                case Url::CONTROLLER_CLIPART :
                    if (!$backgroundId = $this->_getBackgroundId($info[1])) {
                        return null;
                    }

                    $request->setModuleName('background')
                        ->setControllerName(Url::CONTROLLER_CLIPART)
                        ->setActionName('view')
                        ->setParam('id', $backgroundId);
    ...
    /**
     * Retrieve background id by identifier
     * @param  string $identifier
     * @return int
     */
    protected function _getBackgroundId($identifier)
    {
        if (is_null($this->_backgroundId)) {
            $background = $this->_backgroundFactory->create();
            $this->_backgroundId = $background->checkIdentifier(
                $identifier,
