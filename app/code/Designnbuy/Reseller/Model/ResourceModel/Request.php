...
class Request extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('designnbuy_reseller_request', 'request_id');
    }

    /**
     * Retrieves Request Store Code from DB by passed id.
     *
     * @param string $id
     * @return string|bool
     */
    public function getRequestStore_codeById($id)
    {
        $adapter = $this->getConnection();
        $select = $adapter->select()
            ->from($this->getMainTable(), 'store_code')
            ->where('request_id = :request_id');
        $binds = ['request_id' => (int)$id];
     ...
     *
     * @param \Magento\Framework\Model\AbstractModel|\Designnbuy\Reseller\Model\Request $object
     * @return $this
     */
