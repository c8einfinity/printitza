...
class Run extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        $data = $this->getRequest()->getClipart();
        $type = '';
        try {
            if (empty($data['type'])) {
                throw new \Exception(__('Clipart import type is not specified.'), 1);
            }

            $_type = ucfirst($data['type']);
            $import = $this->_objectManager->create('\Designnbuy\Clipart\Model\Import\\'.$_type);
            $type = $data['type'];
            ...
            if ($stats->getData('imported_count')) {
                if (!$stats->getData('skipped_count')) {
                    $this->messageManager->addSuccess(__(
                        'The import process was completed successfully. %1 cliparts and %2 categories where imported.',
                        $stats->getData('imported_cliparts_count'),
                        $stats->getData('imported_categories_count')
                    ));
                } else {
                    $this->messageManager->addNotice(__(
                        'The import process completed. %1 cliparts and %2 categories and %3 tags where imported. Some cliparts or categories or tags where skipped.<br/> %3 %4',
                        $stats->getData('imported_cliparts_count'),
                        $stats->getData('imported_categories_count'),
                        $stats->getData('imported_tags_count'),
                        $stats->getData('skipped_cliparts') ? __('Skipped Cliparts') . ': '. implode(', ', $stats->getData('skipped_cliparts')) . '.<br/>' : '',
                        $stats->getData('skipped_cliparts') ? __('Skipped Categories') . ': '. implode(', ', $stats->getData('skipped_categories')) . '. ' : '',
                        $stats->getData('skipped_cliparts') ? __('Skipped Tags') . ': '. implode(', ', $stats->getData('skipped_tags')) . '. ' : ''
                    ));
                }
            } else {
                if (!$stats->getData('skipped_count')) {
