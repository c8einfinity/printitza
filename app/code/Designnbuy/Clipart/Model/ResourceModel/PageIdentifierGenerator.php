...
class PageIdentifierGenerator
{
    ...
    public function generate(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        while (true) {
            ...
            $clipartId = $clipart->checkIdentifier($finalIdentifier, $object->getStoreId());
            $categoryId = $category->checkIdentifier($finalIdentifier, $object->getStoreId());

            if (!$clipartId && !$categoryId) {
                break;
            } else {
                if ($clipartId
                    && $clipartId == $object->getId()
                    && $object instanceof \Designnbuy\Clipart\Model\Clipart
                ) {
                    break;
                }
                ...
                if ($categoryId
                    ...
                    && $object instanceof \Designnbuy\Clipart\Model\Category
                ) {
                    break;
                }
            }
