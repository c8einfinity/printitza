...
namespace Designnbuy\Designidea\Controller;

use \Designnbuy\Designidea\Model\Url;
/**
 * Designidea Controller Router
 */
class Router implements \Magento\Framework\App\RouterInterface
{
     ...
     * Author factory
     *
     * @var \Designnbuy\Designidea\Model\AuthorFactory
     */
     ...
     * Tag factory
     *
     * @var \Designnbuy\Designidea\Model\TagFactory
     */
     ...
     * @param \Designnbuy\Designidea\Model\AuthorFactory $authorFactory
     * @param \Designnbuy\Designidea\Model\TagFactory $tagFactory
    ...
    public function __construct(
       ...
       /* \Designnbuy\Designidea\Model\AuthorFactory $authorFactory,
        \Designnbuy\Designidea\Model\TagFactory $tagFactory,*/
    ...
    /**
     * Validate and Match Designidea Pages and modify request
     *
    ...
    public function match(\Magento\Framework\App\RequestInterface $request)
    {
        ...
        $designideaRoute = $this->_url->getRoute();

        if ($pathInfo[0] != $designideaRoute) {
            return;
        }
        unset($pathInfo[0]);
            ...
            case Url::PERMALINK_TYPE_SHORT:
                if (isset($pathInfo[1])) {
                    if ($pathInfo[1] == $this->_url->getRoute(Url::CONTROLLER_SEARCH)) {
                        ...
                        } elseif ($designideaId = $this->_getDesignideaId($pathInfo[1])) {
                            $pathInfo[2] = $pathInfo[1];
                        ...
                        if ($designideaId = $this->_getDesignideaId(implode('/', $pathInfo))) {
        ...
        $this->_eventManager->dispatch(
            'designnbuy_designidea_controller_router_match_before',
        ...
        if (!$identifier) {
            $request->setModuleName('designidea')->setControllerName('index')->setActionName('index');
            $success = true;
        } elseif (count($info) > 1) {
            ...
            switch ($info[0]) {
                case Url::CONTROLLER_DESIGNIDEA :
                    if (!$designideaId = $this->_getDesignideaId($info[1])) {
                        return null;
                    }

                    $request->setModuleName('designidea')
                        ->setControllerName(Url::CONTROLLER_CLIPART)
                        ->setActionName('view')
                        ->setParam('id', $designideaId);
    ...
    /**
     * Retrieve designidea id by identifier
     * @param  string $identifier
     * @return int
     */
    protected function _getDesignideaId($identifier)
    {
        if (is_null($this->_designideaId)) {
            $designidea = $this->_designideaFactory->create();
            $this->_designideaId = $designidea->checkIdentifier(
                $identifier,
