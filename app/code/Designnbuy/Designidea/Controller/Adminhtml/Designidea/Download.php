 ...
 */
class Download extends \Designnbuy\Designidea\Controller\Adminhtml\Designidea
{
    /**
     * @var StoreFactory
     */
    ...
    public function execute()
    {
        ...
        if ($id) {
            ...
            $savedSvg = explode(",", $designidea->getSvg());
            $svgImagePath = $this->dnbBaseHelper->getDesignIdeaSVGDir();
            foreach($savedSvg as $svg){
                if(file_exists($svgImagePath) && $svg!=''){
                    if(file_exists($svgImagePath.$svg)){
                        if(!file_exists($svgImagePath.'designidea_'.$id)){
                            mkdir($svgImagePath.'designidea_'.$id, 0777);
                        }
                        copy($svgImagePath.$svg,$svgImagePath.'designidea_'.$id.'/'.$svg);
                    }
                    ...
                    foreach ($doc->getElementsByTagNameNS('http://www.w3.org/2000/svg', 'svg') as $element):
                        foreach ($element->getElementsByTagName("*") as $tags):
                            if($tags->localName=='image' && $tags->getAttribute('xlink:href')!=''):
                                ...
                                if(isset($uploadedImage[0]) && isset($uploadedImage[1])){
                                    if($uploadedImage[0]!='' && file_exists($this->dnbBaseHelper->getMediaPath().$uploadedImage[1])):
                                        ...
                                        copy($this->dnbBaseHelper->getMediaPath().$uploadedImage[1],$svgImagePath.'designidea_'.$id.'/'.$imgName);
                                    endif;
                                }
                            endif;
                        endforeach;
            ...
            //echo 'here'; exit;
            $outputPath = $svgImagePath.'designidea_'.$id.'/';
            $destination = $svgImagePath.'designidea_'.$id.'.zip';
            $zip = new \ZipArchive();
