...
use Designnbuy\Designidea\Api\Data\Category\CategoryAttributeInterface;
use Designnbuy\Designidea\Api\Category\CategoryAttributeGroupRepositoryInterface;
use Designnbuy\Designidea\Api\Category\CategoryAttributeRepositoryInterface;
use Designnbuy\Designidea\Model\Locator\LocatorInterface;
use Designnbuy\Designidea\Model\Category;
use Designnbuy\Designidea\ResourceModel\Eav\Category\Attribute as EavAttribute;
use Designnbuy\Designidea\Model\ResourceModel\Category\Eav\AttributeFactory as EavAttributeFactory;
...
class Eav extends AbstractModifier
{
    ...
    {
        if (!$this->locator->getDesignIdeaCategory()->getId() && $this->dataPersistor->get('designnbuy_designidea_eav_attribute')) {
            return $this->resolvePersistentData($data);
        }

        $categoryId = $this->locator->getDesignIdeaCategory()->getId();
    ...
    {
        return (string)$this->request->getParam('type', $this->locator->getDesignIdeaCategory()->getTypeId());
    }
    ...
    {
        return $this->locator->getDesignIdeaCategory()->getAttributeSetId();
    }
    ...
    private function addUseDefaultValueCheckbox(CategoryAttributeInterface $attribute, array $meta)
    {
        ...
        if ($canDisplayService) {
            ...
            $meta['arguments']['data']['config']['disabled'] = !$this->scopeOverriddenValue->containsValue(
                \Designnbuy\Designidea\Api\Data\Category\CategoryInterface::class,
                $this->locator->getDesignIdeaCategory(),
    ...
    {
        $category = $this->locator->getDesignIdeaCategory();
