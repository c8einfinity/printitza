 ...
 * Background observer
 */
class SaveTemplateData implements ObserverInterface
{
    /**
    ...
    public function __construct(
        ...
        \Designnbuy\Template\Model\ResourceModel\Template\Product $product
    ) {
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        $links = isset($links) ? $links : ['template' => []];
        
        //$links = isset($links['links']) ? $links['links'] : ['printingmethod' => []];
        if (is_array($links)) {
            foreach (['template'] as $linkType) {
