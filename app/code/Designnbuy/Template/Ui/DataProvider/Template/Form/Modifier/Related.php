...
class Related extends AbstractModifier
{
    ...
    public function modifyMeta(array $meta)
    {
        ...
        /* unset($meta['tool']);
        unset($meta['create_tool_modal']);
        unset($meta['create_category_modal']);
        unset($meta['template-settings']);
        unset($meta['websites']);
        unset($meta['general']);
        unset($meta['content']); */
    ...
    public function modifyData(array $data)
    {
        ...
        $items = [];
        $collection = $template->getRelatedProductCollection()->addAttributeToSelect('name');
        ...
        $productTemplate = ObjectManager::getInstance()->create('Magento\Catalog\Model\Product')->getCollection();
        $productTemplate->addAttributeToSelect('name');
        $productTemplate->addAttributeToFilter('template_id',$templateId);

        foreach($productTemplate as $prodData){
            $items[] = [
                'id' => $prodData->getId(),
                'name' => $prodData->getName(),
                'assign' => 'Product',
                'do_we_hide_it' => false,
            ];
        }
        
        $data[$templateId]['links']['product'] = $items;

        $data[$templateId]['current_template_id'] = $templateId;
        $data[$templateId]['current_store_id'] = $this->locator->getStore()->getId();
        //$data[$templateId]['do_we_hide_it'] = false;
