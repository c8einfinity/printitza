...
use Designnbuy\Template\Api\TemplateAttributeGroupRepositoryInterface;
use Designnbuy\Template\Api\TemplateAttributeRepositoryInterface;
use Designnbuy\Template\Model\Locator\LocatorInterface;
use Designnbuy\Template\Model\Template;
use Designnbuy\Template\ResourceModel\Eav\Attribute as EavAttribute;
use Designnbuy\Template\Model\ResourceModel\Eav\AttributeFactory as EavAttributeFactory;
...
use Designnbuy\Template\Ui\DataProvider\EavValidationRules;
...
class Eav extends AbstractModifier
{
    ...
    /**
     * @var TemplateAttributeGroupRepositoryInterface
     */
    ...
    /**
     * @var TemplateAttributeRepositoryInterface
     */
     ...
     * @param TemplateAttributeGroupRepositoryInterface $attributeGroupRepository
     * @param TemplateAttributeRepositoryInterface $attributeRepository
    ...
    public function __construct(
        ...
        TemplateAttributeGroupRepositoryInterface $attributeGroupRepository,
        TemplateAttributeRepositoryInterface $attributeRepository,
    ...
    public function modifyMeta(array $meta)
    {
        ...
        foreach ($this->getGroups() as $groupCode => $group) {
            ...
            if ($attributes) {
                ...
                $meta[$groupCode]['arguments']['data']['config']['dataScope'] = self::DATA_SCOPE_TEMPLATE;
    ...
    protected function setAttributesToEliminate()
    {
        if ($this->getDesignType() == 'template') {
            $this->attributesToEliminate = ['layout_side'];
        }
    }
     ...
     * Get attributes meta
     *
     * @param TemplateAttributeInterface[] $attributes
     ...
     * @param TemplateAttributeInterface $attribute
     * @param string $groupCode
     * @param int $sortOrder
     * @return array
     * @api
    ...
    public function addContainerChildren(
        ...
        TemplateAttributeInterface $attribute,
        $groupCode,
        $sortOrder
    ) {
     ...
     * @api
     */
    public function getContainerChildren(TemplateAttributeInterface $attribute, $groupCode, $sortOrder)
    {
    ...
    {
        if (!$this->locator->getTemplate()->getId() && $this->dataPersistor->get('designnbuy_template_eav_attribute')) {
            return $this->resolvePersistentData($data);
        }

        $templateId = $this->locator->getTemplate()->getId();


        /** @var TemplateAttributeInterface[] $attributes */
        ...
        foreach (array_keys($this->getGroups()) as $groupCode) {
            ...
            foreach ($attributes as $attribute) {
                if (null !== ($attributeValue = $this->setupAttributeData($attribute))) {
                    $data[$templateId][self::DATA_SOURCE_DEFAULT][$attribute->getAttributeCode()] = $attributeValue;
                }
            }
        }
        return $data;
    ...
    private function resolvePersistentData(array $data)
    {
        ...
        $persistentData = (array)$this->dataPersistor->get('designnbuy_template_eav_attribute');
        $this->dataPersistor->clear('designnbuy_template_eav_attribute');
        $templateId = $this->locator->getTemplate()->getId();

        if (empty($data[$templateId][self::DATA_SOURCE_DEFAULT])) {
            $data[$templateId][self::DATA_SOURCE_DEFAULT] = [];
        }

        $data[$templateId] = array_replace_recursive(
            $data[$templateId][self::DATA_SOURCE_DEFAULT],
            $persistentData
        );
        return $data;
    ...
    {
        return (string)$this->request->getParam('type', $this->locator->getTemplate()->getTypeId());
    }
    ...
    private function getGroups()
    {
        if (!$this->attributeGroups) {
            ...
            foreach ($attributeGroupSearchResult->getItems() as $group) {
                ...
                /*if($group->getAttributeGroupCode() == 'design-tool-settings' && $this->getDesignType() == 'template'){
                    continue;
                }*/
    ...
    {
        return $this->locator->getTemplate()->getAttributeSetId();
    }

    /**
     * Retrieve attributes
     *
     * @return TemplateAttributeInterface[]
     */
    ...
    /**
     * Loading template attributes from group
     *
    ...
    private function loadAttributes(AttributeGroupInterface $group)
    {
        ...
        $searchCriteria = $this->searchCriteriaBuilder
            ...
            ->addFilter(TemplateAttributeInterface::IS_VISIBLE, 1)
     ...
     * @api
     */
    public function setupAttributeMeta(TemplateAttributeInterface $attribute, $groupCode, $sortOrder)
    {
     ...
     * @return array
     */
    private function addUseDefaultValueCheckbox(TemplateAttributeInterface $attribute, array $meta)
    {
        ...
        if ($canDisplayService) {
            ...
            $meta['arguments']['data']['config']['disabled'] = !$this->scopeOverriddenValue->containsValue(
                \Designnbuy\Template\Api\Data\TemplateInterface::class,
                $this->locator->getTemplate(),
     ...
     * @api
     */
    public function setupAttributeContainerMeta(TemplateAttributeInterface $attribute)
    {
     ...
     * @api
     */
    public function setupAttributeData(TemplateAttributeInterface $attribute)
    {
     ...
     * @return array
     */
    private function customizeCheckbox(TemplateAttributeInterface $attribute, array $meta)
    {
     ...
     * @return array
     */
    private function customizeImageAttribute(TemplateAttributeInterface $attribute, array $meta)
    {
        $template = $this->locator->getTemplate();
        $templateData = array();
        /*if ($attribute->getFrontendInput() === 'image' || $attribute->getFrontendInput() === 'file') {
            $fileData = $template->getData($attribute->getAttributeCode());
            if($fileData != ''){
                $meta['arguments']['data']['config']['elementTmpl'] = 'Designnbuy_Template/image';
            }
        }*/
    ...
    private function overrideFileUploaderMetadata(
        ...
        if (in_array($attribute->getFrontendInput(), $this->fileUploaderTypes)) {
            ...
            $meta['arguments']['data']['config']['component'] = 'Designnbuy_Template/js/components/file-uploader';
            $meta['arguments']['data']['config']['previewTmpl'] = 'Designnbuy_Template/components/file-uploader';
    ...
    {
        $url = 'template/template_image/upload';
        return $url;
    }
     ...
     * @return array
     */
    private function customizePriceAttribute(TemplateAttributeInterface $attribute, array $meta)
    {
     ...
     * @return array
     */
    private function customizeWysiwyg(TemplateAttributeInterface $attribute, array $meta)
    {
        if (!$attribute->getIsWysiwygEnabled()) {
            return $meta;
        }
     ...
     * @return mixed
     */
    private function getValue(TemplateAttributeInterface $attribute)
    {
        $attributeData = array();
        ...
        return $template->getData($attribute->getAttributeCode());
    }
    /**
    ...
    private function getFileUploaderData(
        ...
        $file = isset($templateData[$attributeCode])
            ? $templateData[$attributeCode]
            : '';
     ...
     */
    private function getScopeLabel(TemplateAttributeInterface $attribute)
    {
        if (
        ...
        switch ($attribute->getScope()) {
            case TemplateAttributeInterface::SCOPE_GLOBAL_TEXT:
                return __('[GLOBAL]');
            case TemplateAttributeInterface::SCOPE_WEBSITE_TEXT:
                return __('[WEBSITE]');
            case TemplateAttributeInterface::SCOPE_STORE_TEXT:
     ...
     * @return bool
     */
    private function canDisplayUseDefault(TemplateAttributeInterface $attribute)
    {
        $attributeCode = $attribute->getAttributeCode();
        /** @var Template $template */
        $template = $this->locator->getTemplate();
        ...
        return $this->canDisplayUseDefault[$attributeCode] = (
            ($attribute->getScope() != TemplateAttributeInterface::SCOPE_GLOBAL_TEXT)
            && $template
            && $template->getId()
            && $template->getStoreId()
        );
    ...
    {
        return $attribute->getScope() === TemplateAttributeInterface::SCOPE_GLOBAL_TEXT;
    }
    ...
    private function getFileProcessorFactory()
    {
        if ($this->fileProcessorFactory === null) {
            $this->fileProcessorFactory = ObjectManager::getInstance()
                ->get('Designnbuy\Template\Model\FileProcessorFactory');
        }
