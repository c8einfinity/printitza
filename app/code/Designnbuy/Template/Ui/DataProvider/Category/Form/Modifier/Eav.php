 ...
 */
namespace Designnbuy\Template\Ui\DataProvider\Category\Form\Modifier;

use Designnbuy\Template\Api\Data\Category\CategoryAttributeInterface;
use Designnbuy\Template\Api\Category\CategoryAttributeGroupRepositoryInterface;
use Designnbuy\Template\Api\Category\CategoryAttributeRepositoryInterface;
use Designnbuy\Template\Model\Locator\LocatorInterface;
use Designnbuy\Template\Model\Category;
use Designnbuy\Template\ResourceModel\Eav\Category\Attribute as EavAttribute;
use Designnbuy\Template\Model\ResourceModel\Category\Eav\AttributeFactory as EavAttributeFactory;
...
class Eav extends AbstractModifier
{
    ...
    {
        if (!$this->locator->getTemplateCategory()->getId() && $this->dataPersistor->get('designnbuy_template_eav_attribute')) {
            return $this->resolvePersistentData($data);
        }

        $categoryId = $this->locator->getTemplateCategory()->getId();
    ...
    {
        return (string)$this->request->getParam('type', $this->locator->getTemplateCategory()->getTypeId());
    }
    ...
    {
        return $this->locator->getTemplateCategory()->getAttributeSetId();
    }
    ...
    private function addUseDefaultValueCheckbox(CategoryAttributeInterface $attribute, array $meta)
    {
        ...
        if ($canDisplayService) {
            ...
            $meta['arguments']['data']['config']['disabled'] = !$this->scopeOverriddenValue->containsValue(
                \Designnbuy\Template\Api\Data\Category\CategoryInterface::class,
                $this->locator->getTemplateCategory(),
    ...
    {
        $category = $this->locator->getTemplateCategory();
