...
class TemplateUrlRewriteGenerator
{
    /**
     * Entity type code
     */
     ...
     */
    public function generate(Template $currenttemplate)
    {
        $this->currenttemplate = $currenttemplate;
        $storeId = $this->currenttemplate->getStoreId();
    ...
    {
        $urls = [];
        $templateId = $this->currenttemplate->getId();
        foreach ($this->currenttemplate->getStoreIds() as $storeId) {
    ...
    protected function generateForSpecificStoreView($storeId)
    {
         ...
         */
        $urls = array_merge(
            $this->canonicalUrlRewriteGenerator->generate($storeId, $this->currenttemplate),
            $this->currentUrlRewritesRegenerator->generate($storeId, $this->currenttemplate)
        );
