 ...
 */
class Template extends \Magento\Catalog\Model\AbstractModel implements
    IdentityInterface,
    TemplateInterface
{
    /**
     ...
     */
    const ENTITY = 'designnbuy_template';

    const CACHE_TAG = 'designnbuy_template';
     ...
     * @var string
     */
    protected $_eventPrefix = 'designnbuy_template_entity';
    ...
    public function __construct(
        ...
        \Designnbuy\Template\Model\Template\Upsell $upsellInstance,
        \Designnbuy\Template\Model\Template\Crosssell $crosssellInstance,
    ...
    {
        parent::_construct();
        $this->_init('Designnbuy\Template\Model\ResourceModel\Template');
    }
    ...
    protected function _afterSave()
    {
        ...
        $this->getUpsellInstance()->saveUpselltemplateRelation($this);
     ...
     */
    public function getCrosssellTemplates()
    {
    ...
    {
        if (!$this->hasData('template_image')) {
            if ($file = $this->getData('image')) {
                $image = $this->getMediaUrl($file);
            } else {
                $image = false;
            }
            $this->setData('template_image', $image);
        }

        return $this->getData('template_image');
    }
    ...
    public function getMediaUrl($file)
    {
        return $this->_storeManager->getStore()
            ->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA) . \Designnbuy\Template\Helper\Template::BASE_MEDIA_PATH . '/' . $file;
    }
    ...
    public function duplicate()
    {
        ...
        $object->setData('website_ids', $websiteIds);
        $relatedProducts = $this->getRelatedProducts();
        
        $linksData = [];
        foreach ($relatedProducts as $item) {
            $linksData['product'][$item->getId()] = [
                'position' => $item->getPosition()
            ];
        }

        if (!is_null($linksData)) {
            $object->setProductsData($linksData);
        }
        return $object->save();
    }
    
    public function migrateTemplate($template_data)
    {
        $object = clone $this;
        $object->setData($template_data);
        $object
            ->unsetData('entity_id')
