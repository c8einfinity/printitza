...
/**
 * Color color save controller
 */
class Save extends \Designnbuy\Color\Controller\Adminhtml\Color
{
    /**
     * Before model save
     * @param  \Designnbuy\Color\Model\Color $model
            ...
            foreach (['color', 'product'] as $linkType) {
                if (!empty($links[$linkType]) && is_array($links[$linkType])) {
                    ...
                    foreach ($links[$linkType] as $item) {
                        $linksData[$item['id']] = [
                            'position' => array_key_exists('position', $item) && isset($item['position']) ? $item['position'] : 0,
                        ];
                    }
            ...
            if (isset($data[$key]) && is_array($data[$key])) {
                if (!empty($data[$key]['delete'])) {
                    ...
                    if (isset($data[$key][0]['name']) && isset($data[$key][0]['tmp_name'])) {
                        ...
                        $model->setData($key, Color::BASE_MEDIA_PATH . DIRECTORY_SEPARATOR . $image);
                        $imageUploader = $this->_objectManager->get(
                            'Designnbuy\Color\ImageUpload'
                        );
            ...
            foreach ($images as $image) {
                if (empty($image['removed'])) {
                    if (!empty($image['value_id'])) {
                        ...
                        $gallery[] = Color::BASE_MEDIA_PATH . DIRECTORY_SEPARATOR . $image['file'];
                    }
                }
            }
