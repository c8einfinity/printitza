...
class SetWeightInCustomHeightWidth implements ObserverInterface
{
    ...
    public function __construct(
        ...
        \Designnbuy\Pricecalculator\Helper\Data $priceHelper
    ) {
    ...
    public function execute(Observer $observer)
    {
        ...
        $productdt = $objectManager->create('Magento\Catalog\Model\Product')->load($product->getId());
        //echo $productdt->getEnableCustomHeightWidth(); exit;
        //echo $productdt->getPricingLimit(); exit;
        ...
        foreach ($options as $optionId => $optionValue) {
            ...
            if($productdt->getOptionById($optionId)->getDesigntoolType() == "width"){
                $widthData = $optionValue;
            }
            
            if($productdt->getOptionById($optionId)->getDesigntoolType() == "height"){
                $heightData = $optionValue;
            }
        ...
        foreach ($options as $optionId => $optionValue) {
            ...
            foreach ($optionValues as $valueId) {
                ...
                if($value->getData('designtool_weight') == "per_unit"){
                    if($productdt->getEnableCustomHeightWidth() && $widthData > 0 && $heightData > 0){
                        $sqareaData = $widthData * $heightData;
                        $optionWeight += $sqareaData * $value->getData(Helper::KEY_WEIGHT);
                    }
                } else {
                    $optionWeight += $value->getData(Helper::KEY_WEIGHT);
                }
            }
            //echo $optionWeight.'<br/>';
        ...
        $heightWidthDt = $this->priceHelper->getMinMaxForCustomSize($productdt);
        if($productdt->getEnableCustomHeightWidth() && isset($heightWidthDt['Weight']) && $widthData > 0 && $heightData > 0){
            $totalsqarea = $widthData * $heightData;
            if($heightWidthDt['Weight'] > 0){
                $weight += $totalsqarea * $heightWidthDt['Weight'];
            }
        }
        //echo $widthData. ' = ' . $heightData; exit;
        ...
        if ($this->isWeightEnabled($product)) {
            //echo $originalWeight .'+'. $weight; exit;
            ...
            //echo $quoteItem->getWeight() . ' = ' . $quoteItem->getRowWeight(); exit;
        }
